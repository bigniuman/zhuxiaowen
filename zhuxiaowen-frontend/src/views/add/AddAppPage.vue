<template>
  <div id="addAppPage">
    <h2 style="margin-bottom: 32px; margin-left: 720px">创建应用</h2>
    <a-form
      :model="form"
      :style="{ width: '480px', marginLeft: '550px' }"
      label-align="left"
      auto-label-width
      @submit="handleSubmit"
    >
      ...
      <a-form-item field="scoringStrategy" label="评分策略">
        ...
        >
          <a-option
            ...
            :label="value"
            :key="key"
            :disabled="isOptionDisabled(Number(key))"
          />
        </a-select>
      </a-form-item>
      <a-form-item>
...
const isOptionDisabled = (optionValue: number) => {
  // 如果应用类型是"分类"(值为0)，且当前选项是"AI"(值为1)，则禁用
  if (form.value.appType === 0 && optionValue === 1) {
    return true;
  }
  return false;
};
